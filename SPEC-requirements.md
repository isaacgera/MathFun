...
- **Audience:** School-going children aged 6-10 (fun-first, not a dry drill).
...
## Scope (v1)
**In scope:** multiplication tables 1x-20x; three difficulty levels + pick-a-specific-table
practice; 4-option multiple-choice answering; 10-question rounds (untimed default + optional
timed mode); encouraging feedback; rewards (stars, streaks, badges); personal-best tracking;
mastery grid; light/dark theme; sound on/off; installable offline PWA.

**Out of scope (v1, noted for later):** other operations (+ - x div), typed/number-pad input,
multiple child profiles, cloud sync, leaderboards. (A separate "Maths Quiz Builder" idea exists
...
- R1.4 The last-used mode is remembered and pre-selected next time (local storage).
...
- R4.3 An optional **"Beat the clock"** timed mode can be turned on for a per-question or per-round timer.
...
- R9.7 A **sound on/off** toggle; the game is fully playable and understandable with sound off.
...
- R10.2 **Light and dark themes** both supported via tokens, respecting `prefers-color-scheme`, with a manual toggle that persists.
...
1. **Distractors:** confirm "believable near-miss" distractors (my recommendation) vs purely random numbers. (R2.3)
2. **Timed mode detail:** per-question timer or whole-round timer for "Beat the clock"? (R4.3)
3. **Star thresholds:** e.g. 3 stars = 10/10, 2 stars = 8-9, 1 star = 6-7? Adjust as you like. (R5.1)
4. **Mastery basis:** how many recent attempts define "mastered" (e.g. last 5 attempts >= 80% correct)? (R6.2)
5. **Sound assets:** synthesized simple tones (no files, keeps it tiny/offline) vs bundled sound files? (R3.1)
6. **Licensing:** which licence for MathFun? (needed before shipping)
